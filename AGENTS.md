...
├── 思维导图任务交接.md            # 一次性任务交接（下一任务：思维导图，完成后归档删除；md-only）
├── index.html                    # 站点首页（由 build_site.py 的 build_root_index 模板生成，非 md 驱动）
...
- **统计数字唯一来源**：`kb_audit.py` 输出；只在 `00_知识库总索引.md` 展示（首页 hero 的数字由 build_root_index 现场统计生成，不手工维护）。
